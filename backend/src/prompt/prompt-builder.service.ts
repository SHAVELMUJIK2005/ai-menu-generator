...
ТРЕБОВАНИЯ К БЛЮДАМ:
- Приоритет №1: блюда должны быть ВКУСНЫМИ и реально съедобными для обычного человека
...
- Не предлагай странные или конфликтующие сочетания вкусов (например рыба + сладкие сиропы, мясо + варенье)
- Основа меню: понятная домашняя кухня, как в обычной семье (без экзотики и экспериментальных сочетаний)
...
Для каждого дня: breakfast, lunch, dinner (можно добавить snack).`;
  ...
  buildSafetyRules(): string {
    return [
      ...
      "12. В каждом основном блюде (обед/ужин) минимум 3 ингредиента, в перекусе 1-4 ингредиента",
      "13. Перед финальным ответом проверь себя: меню должно быть аппетитным, без странных сочетаний и повторов",
      "14. Не используй напитки как полноценный обед или ужин",
    ].join("\n");
  }
